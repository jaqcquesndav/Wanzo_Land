...
export function ManagementPage() {
  ...
  const group = appGroups.find(g => g.id === 'management')!;
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/60 to-black/30 flex items-center">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-white">
            <h1 className="text-4xl font-bold tracking-tight">
              {selectedApp ? selectedApp.name : 'ERP'}
            </h1>
            <p className="mt-4 text-xl text-gray-200 max-w-2xl">
              {selectedApp ? selectedApp.description : 'Une suite complète d\'applications pour gérer efficacement votre entreprise'}
            </p>
          </div>
        </div>
      </div>
